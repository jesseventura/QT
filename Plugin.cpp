...
#define PLUGIN_NAME "MyQuoteTrackerData"
#define VENDOR_NAME "itotimocheen@gmail.com"
#define PLUGIN_VERSION 20001
#define PLUGIN_ID PIDCODE( 'M', 'Y', 'Q', 'T')
...
struct RecentInfo *FindOrAddRecentInfo( LPCTSTR pszTicker )
{
	...
	int iSymbol = 0;
	for( ; g_aInfos && iSymbol < RecentInfoSize && g_aInfos[ iSymbol ].Name && g_aInfos[ iSymbol ].Name[ 0 ]; iSymbol++ )
	{
		if( ! stricmp( g_aInfos[ iSymbol ].Name, pszTicker ) )
		{
			ri = &g_aInfos[ iSymbol ];
